...
function getAuthMode() {
    ...
    throw new Error(`Valor inválido para OCI_AUTH_MODE: ${rawMode}. Valores aceitos: config, apikey, instance_principal, session`);
}
const authMode = getAuthMode();
const OCI_READ_ONLY = parseBoolean(process.env.OCI_READ_ONLY, true);
function hasTenantId(provider) {
    return typeof provider.getTenantId === 'function';
}
/**
 ...
 */
function resolveCompartmentId(provider, argCompartmentId) {
    if (argCompartmentId) {
    ...
    throw new Error('compartmentId não informado. No modo instance_principal não é possível resolver o compartment raiz automaticamente — informe compartmentId explicitamente (use list_compartments para descobrir o OCID).');
}
async function buildProvider() {
    ...
    switch (authMode) {
        ...
        case 'apikey': {
            ...
            return new common.SimpleAuthenticationDetailsProvider(tenancy, user, fingerprint, privateKey, passphrase, common.Region.fromRegionId(region));
        }
    }
}
 ...
 */
async function getClients(regionId) {
    ...
    const result = { provider, compute, vcn, objectStorage, identity: identityClient };
    clientsByRegion.set(cacheKey, result);
    return result;
}
let namespaceCache = null;
async function resolveNamespace(objectStorage, argNamespace) {
    if (argNamespace) {
        return argNamespace;
    }
    if (namespaceCache) {
...
function toInstanceAction(action) {
    const normalized = action.toUpperCase();
    if (!VALID_INSTANCE_ACTIONS.has(normalized)) {
        throw new Error(`Ação inválida: ${action}. Valores aceitos: ${[...VALID_INSTANCE_ACTIONS].join(', ')}`);
    }
    return normalized;
}
// ---------------------------------------------------------------------------
...
function summarizeVnicAttachment(v) {
    return {
        id: v.id,
...
function summarizeSecurityList(sl) {
    return {
        id: sl.id,
...
function summarizeRouteTable(rt) {
    return {
        id: rt.id,
...
function createServer() {
    const server = new Server({ name: 'mcp-oci', version: '1.0.0' }, { capabilities: { tools: {} } });
    // -------------------------------------------------------------------------
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            {
                name: 'list_regions',
                description: 'Lists the OCI regions this tenancy is subscribed to (region ID, region key, home region flag). Use this to resolve a region name (e.g. "Vinhedo", "São Paulo") to its region ID (e.g. "sa-vinhedo-1") before passing it as the region argument to other tools.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        tenancyId: {
            ...
            },
            {
                ...
                description: 'Lists compartments by name and/or parent compartment. Use this to resolve a compartment name (e.g. "cpLinux") to its OCID before calling other tools. Defaults to searching the entire tenancy tree from the root compartment.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        name: {
            ...
            },
            {
                ...
                description: 'Performs a lifecycle action on a Compute instance (START, STOP, SOFTSTOP, RESET, SOFTRESET, SENDDIAGNOSTICINTERRUPT). Blocked when OCI_READ_ONLY=true.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        instanceId: { type: 'string', description: 'Instance OCID' },
            ...
            },
            {
                ...
                description: 'Lists security lists in a compartment, optionally filtered by VCN. Returns a trimmed summary (with ingress/egress rule counts) per list unless raw=true — pass raw=true to inspect the actual rules.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        compartmentId: {
            ...
            },
            {
                ...
                description: 'Lists route tables in a compartment, optionally filtered by VCN. Returns a trimmed summary (with route rule count) per table unless raw=true — pass raw=true to inspect the actual routes.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        compartmentId: {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            switch (name) {
                case 'list_regions': {
                    const tenancyId = resolveCompartmentId(provider, args?.tenancyId);
                ...
                case 'list_compartments': {
                    const parentCompartmentId = resolveCompartmentId(provider, args?.compartmentId);
                    const compartmentName = args?.name;
                    const compartmentIdInSubtree = args?.includeSubtree ?? true;
                ...
                case 'list_instances': {
                    const compartmentId = resolveCompartmentId(provider, args?.compartmentId);
                    const availabilityDomain = args?.availabilityDomain;
                ...
                case 'instance_action': {
                    const instanceId = args?.instanceId;
                    const action = toInstanceAction(args?.action);
                ...
                case 'list_buckets': {
                    const compartmentId = resolveCompartmentId(provider, args?.compartmentId);
                    const namespaceName = await resolveNamespace(objectStorage, args?.namespace);
                ...
                case 'create_bucket': {
                    ...
                    const compartmentId = resolveCompartmentId(provider, args?.compartmentId);
                    const namespaceName = await resolveNamespace(objectStorage, args?.namespace);
                    const publicAccessType = args?.publicAccessType ??
